...
from wtforms import TextAreaField, StringField, SubmitField, StringField, PasswordField, RadioField, IntegerField
from flask_wtf.file import FileField, FileAllowed, FileRequired
from wtforms.validators import DataRequired, Length, Email

class NewsForm(FlaskForm):
    news = TextAreaField('Enter News', validators=[DataRequired(), Length(min=2, max=100)])
    link = StringField('Add a link', validators=[Length(max=100)])
    link_text = StringField('Add text to display above the link', validators=[Length(max=100)])
...
class ConsultancyForm(FlaskForm):
    title = StringField('Add Title of Consultancy', validators=[Length(max=100)])
    guide = StringField('Add Name of the Guide/ Investigator', validators=[Length(max=100)])
    agency = StringField('Add Funding Agency ', validators=[Length(max=100)])
    student = StringField('Add Name of the Student Involved', validators=[Length(max=100)])
    status = StringField('Status/ Amt.', validators=[Length(max=100)])
